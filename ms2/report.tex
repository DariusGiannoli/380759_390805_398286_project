...
$40$ epochs. The training curve (Fig.~\ref{fig:mlp_loss}) converges
...
\begin{figure}[t!]
    ...
    \begin{subfigure}[t]{0.6\columnwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/mlp_loss.png}
        \caption{Training cross-entropy of the selected model.}
        \label{fig:mlp_loss}
    \end{subfigure}
...
Fig.~\ref{fig:km_init} sweeps four init strategies at the selected $K$
($20$ seeds each). K-Means++ and best-of-$10$ restarts cut inertia
variance ($\sigma{\approx}17$ for \textit{km++}{$\times$}$10$ vs
$\sigma{\approx}46$ for plain random), confirming they find
geometrically better optima --- yet all four strategies land within
$0.03$~F1 of one another, well within their own seed-to-seed std
($\sim\!0.07$). The K-Means objective (inertia) and the downstream
voting objective (macro-F1) are simply not aligned on imbalanced
classes.
...
$0.02$ of MS1's $k$-NN ($0.623$) --- the same order of magnitude as
the $\sim\!0.08$ seed-to-seed std seen in Fig.~\ref{fig:km_init} ---
but $0.19$ below the MLP. The confusion matrices (Fig.~\ref{fig:cm})
localise that gap: K-Means catches Low (recall $0.94$) and even High
(recall $0.85$) acceptably, but \emph{collapses on Medium}
(recall $0.16$; $84/107$ Medium samples voted as Low). The MLP is much
more balanced across the three classes ($0.94/0.69/0.69$). So the
MLP--K-Means gap is essentially a Medium-class story --- different
from the High-class gap that drove MS1's parametric--vs--non-parametric
margin --- and a direct symptom of unsupervised partitioning
mis-aligning with class boundaries on imbalanced data.
